...
class FeatureListAdapter(private val listeners: ClickListenersCallback<Feature>):BaseAdapter<FeatureState, FeatureListItemBinding>(R.layout.feature_list_item) {
    override fun bind(item: FeatureState, binding: FeatureListItemBinding) {
        binding.feature = item.feature
        binding.listeners = listeners
        binding.state = item.state
        binding.featureProgressbar.progressDone(item.feature.startingDate.time, Date().time, item.feature.endingDate.time)
    }
